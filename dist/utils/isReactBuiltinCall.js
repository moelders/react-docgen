...
function isReactBuiltinCall(path, name) {
  ...
  if (_astTypes.namedTypes.CallExpression.check(path.node)) {
    ...
    _astTypes.namedTypes.MemberExpression.check(value.node) && _astTypes.namedTypes.Identifier.check(value.get('property').node) && value.get('property').node.name === name ||
